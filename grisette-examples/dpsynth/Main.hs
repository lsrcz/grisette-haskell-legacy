  ...
  masIntSynthedOpt1 :: Maybe (ConProgram (IntN 11)) <-
    timeItAll "masOpt1" $
      synth1
        ...
        availableBinary
        ()
        (\x -> let t = foldl (\acc y -> acc &&~ y >=~ -8 &&~ y <=~ 8) (conc True) $ getSingle <$> join x in trace (show t) t)
        (masSpec @(IntN 11) . fmap (getSingle . head))
        masSketchOpt
  ...
  masIntSynthedOpt :: Maybe (ConProgram Integer) <-
    timeItAll "masOpt" $
      synth1
        ...
        ()
        (const $ conc True)
        (masSpec @Integer . fmap (getSingle . head))
        masSketchOpt
  ...
  masIntSynthed1 :: Maybe (ConProgram (IntN 10)) <-
    timeItAll "mas1" $
      synth1
        ...
        availableBinary
        ()
        (\x -> foldl (\acc y -> acc &&~ y >=~ -4 &&~ y <=~ 4) (conc True) $ getSingle <$> join x)
        (masSpec @(IntN 10) . fmap (getSingle . head))
        masSketch
