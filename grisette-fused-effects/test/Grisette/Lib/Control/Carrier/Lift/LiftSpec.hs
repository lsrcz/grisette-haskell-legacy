  ...
  describe "Lift" $ do
    it "Mergeable for Lift" $ do
      runM (lm (SSBool "c") 1 2 :: R)
        `shouldBe` mrgIf (SSBool "c") (return 1) (return 2)
    ...
    it "SimpleMergeable1 for Lift" $ do
      runM (ls (SSBool "c") (SSBool "a") (SSBool "b") :: RB)
        `shouldBe` mrgReturn (ITE (SSBool "c") (SSBool "a") (SSBool "b"))
    it "UnionLike for Lift" $ do
      ...
      runM (lu (SSBool "c") 1 1 :: R)
        `shouldBe` mrgReturn 1
      runM (lu' (SSBool "c") 1 2 :: R)
        `shouldBe` unionIf (SSBool "c") (return 1) (return 2)
      runM (lu' (SSBool "c") 1 1 :: R)
        `shouldBe` unionIf (SSBool "c") (return 1) (return 1)
