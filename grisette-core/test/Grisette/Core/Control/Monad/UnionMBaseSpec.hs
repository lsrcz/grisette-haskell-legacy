...
module Grisette.Core.Control.Monad.UnionMBaseSpec where
...
spec = do
  ...
  describe "Evaluate for UnionMBase" $ do
    it "Evaluate for UnionMBase should work" $ do
      ...
      evaluate False model (mrgSingle $ SSBool "a") `shouldBe` (mrgSingle $ SSBool "a" :: UnionMBase SBool SBool)
      evaluate True model (mrgSingle $ SSBool "a") `shouldBe` (mrgSingle $ CBool False :: UnionMBase SBool SBool)
      evaluate False model1 (mrgSingle $ SSBool "a") `shouldBe` (mrgSingle $ CBool True :: UnionMBase SBool SBool)
      evaluate True model1 (mrgSingle $ SSBool "a") `shouldBe` (mrgSingle $ CBool True :: UnionMBase SBool SBool)
      evaluate False model1 (mrgIf (SSBool "a") (mrgSingle $ Left (SSBool "d")) (mrgSingle $ Right (SSBool "e")))
        `shouldBe` (mrgSingle $ Left $ SSBool "d" :: UnionMBase SBool (Either SBool SBool))
      evaluate True model1 (mrgIf (SSBool "a") (mrgSingle $ Left (SSBool "d")) (mrgSingle $ Right (SSBool "e")))
        `shouldBe` (mrgSingle $ Left $ CBool False :: UnionMBase SBool (Either SBool SBool))
      evaluate False model1 (mrgIf (SSBool "d") (mrgSingle $ Left (SSBool "a")) (mrgSingle $ Right (SSBool "b")))
        `shouldBe` ( mrgIf (SSBool "d") (mrgSingle $ Left $ CBool True) (mrgSingle $ Right $ CBool False) ::
                       UnionMBase SBool (Either SBool SBool)
                   )
      evaluate True model1 (mrgIf (SSBool "d") (mrgSingle $ Left (SSBool "a")) (mrgSingle $ Right (SSBool "b")))
        `shouldBe` (mrgSingle $ Right $ CBool False :: UnionMBase SBool (Either SBool SBool))
      evaluate False model1 (mrgIf (SSBool "a") (mrgSingle $ Left (SSBool "b")) (mrgSingle $ Right (SSBool "c")))
        `shouldBe` (mrgSingle $ Left $ CBool False :: UnionMBase SBool (Either SBool SBool))
