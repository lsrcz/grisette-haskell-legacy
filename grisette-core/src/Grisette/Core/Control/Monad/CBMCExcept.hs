...
deriving newtype instance (Evaluate model a, Evaluate model b) => Evaluate model (CBMCEither a b)
...
instance (Functor m, Monad m, Monoid e) => Alternative (CBMCExceptT e m) where
  ...
  CBMCExceptT mx <|> CBMCExceptT my = CBMCExceptT $ do
    ex <- mx
    case ex of
      CBMCEither (Left e) -> liftM (cbmcEither (CBMCEither . Left . mappend e) (CBMCEither . Right)) my
...
instance MonadTrans (CBMCExceptT e) where
  lift = CBMCExceptT . liftM (CBMCEither . Right)
  {-# INLINE lift #-}
...
instance (Evaluate model (m (CBMCEither e a))) => Evaluate model (CBMCExceptT e m a) where
  evaluate fillDefault model (CBMCExceptT v) = CBMCExceptT $ evaluate fillDefault model v
  {-# INLINE evaluate #-}
