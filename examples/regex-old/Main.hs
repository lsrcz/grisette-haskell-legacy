...
mrgSuspend ::
  forall m s bool x.
  (Monad m, Functor s, MonadUnion bool m, SymBoolOp bool, Mergeable bool x, Mergeable1 bool s) =>
  s (Coroutine s m x) ->
  Coroutine s m x
mrgSuspend s = withMergeable @bool @s @(Coroutine s m x) $ Coroutine $ mrgReturn @bool (Left s)

mrgYield :: (Monad m, SymBoolOp bool, MonadUnion bool m, Mergeable bool x) => x -> Coroutine (Yield x) m ()
mrgYield x = mrgSuspend (Yield x $ mrgReturn ())

mrgMapSuspension ::
  forall s m bool x s'.
  (Functor s, Monad m, SymBoolOp bool, MonadUnion bool m, Mergeable bool x, Mergeable1 bool s') =>
  (forall y. s y -> s' y) ->
  Coroutine s m x ->
  Coroutine s' m x
...
simpleTransducer ::
  (Monad m, SymBoolOp bool, MonadUnion bool m, Mergeable bool a, Mergeable bool x) =>
...
weaveYieldTransducer ::
  (Monad m, SymBoolOp bool, MonadUnion bool m, Mergeable bool b) =>
